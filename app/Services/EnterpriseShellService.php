...
class EnterpriseShellService
{
    ...
    {
        $groups = [
            'Visão Geral' => [['dashboard','fa-house','Início']],
            'Pessoas & Estrutura' => [
                ['colaboradores','fa-users','Colaboradores'],['unidades','fa-building','Unidades'],['setores','fa-sitemap','Setores'],['cargos','fa-briefcase','Cargos e Funções'],['badge-studio','fa-id-card','Crachás & Modelos'],
            ],
            'Identidade & Acessos' => [
                ['identity-directory','fa-address-book','Identidade e Diretório'],['perfis','fa-shield-halved','Perfis de Acesso'],['sistemas_hc','fa-diagram-project','Aplicações SSO'],['administradores','fa-user-shield','Administradores'],
            ],
            'Automação & Integrações' => [
                ['automacoes','fa-bolt','Automações e Jornada'],['cloud_status','fa-cloud','Saúde das Integrações'],['comunicacoes','fa-comments','Comunicações'],
            ],
            'Governança & Auditoria' => [
                ['relatorios','fa-chart-column','Relatórios'],['auditoria','fa-file-shield','Trilha de Auditoria'],['erros','fa-triangle-exclamation','Diagnósticos e Falhas'],
            ],
            'Administração' => [
                ['configuracoes','fa-sliders','Configurações Mestres'],['changelog','fa-clock-rotate-left','Notas de Versão'],
            ],
        ];
        ...
        $out .= '<div class="v4-search"><i class="fa-solid fa-magnifying-glass"></i><input type="search" id="v4ModuleSearch" placeholder="Buscar módulo…"><kbd>/</kbd></div><nav class="v4-nav">';
        foreach ($groups as $title => $items) {
            $active = false;
            foreach ($items as $item) { if ($item[0] === $page) { $active = true; break; } }
    ...
    {
        $titles = [
            'dashboard'=>['Visão Geral','Acompanhe pessoas, acessos e integrações em um só lugar.'],'colaboradores'=>['Colaboradores','Gestão completa do ciclo de vida das pessoas.'],'unidades'=>['Unidades','Estrutura organizacional e configurações por unidade.'],'setores'=>['Setores','Organização dos setores e responsabilidades.'],'cargos'=>['Cargos e Funções','Funções, responsabilidades e políticas de acesso.'],'identity-directory'=>['Identidade e Diretório','LDAP, Active Directory e sincronização corporativa.'],'perfis'=>['Perfis de Acesso','Governança de permissões e RBAC.'],'sistemas_hc'=>['Aplicações SSO','Aplicações corporativas e acesso centralizado.'],'administradores'=>['Administradores','Operadores e privilégios administrativos.'],'automacoes'=>['Automações e Jornada','Onboarding, movimentações e desligamentos.'],'cloud_status'=>['Saúde das Integrações','Microsoft 365, Google, LDAP e WhatsApp.'],'comunicacoes'=>['Comunicações','E-mails, WhatsApp e histórico de entregas.'],'relatorios'=>['Relatórios','Indicadores, exportações e visão de governança.'],'auditoria'=>['Trilha de Auditoria','Eventos, operadores e rastreabilidade.'],'erros'=>['Diagnósticos e Falhas','Erros operacionais e saúde técnica.'],'configuracoes'=>['Configurações Mestres','Parâmetros globais, integrações e segurança.'],'changelog'=>['Notas de Versão','Histórico completo de evolução do CADCOLAB.'],
        ];
